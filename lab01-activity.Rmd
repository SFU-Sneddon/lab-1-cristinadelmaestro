...
```{bash, engine.opts='-l'}
pwd
#/local-scratch/localhome/cdelmaes/lab-1-cristinadelmaestro
```
...
**Task** 
* Duplicate the last line in this code chunk and modify it by replacing the single `>` with `>>`. What happens when you rerun it? --> The single > redirected the output of the head command to a new file named genotype_head.txt. Whereas the >> appended output of the head command to the genotype_head.txt file such that it added another 25 lines of the output of the head command to the original file.

* What happens if you change the output file name (after `>>`) to `~/MBB243/out/genotype_head.txt`? --> I get an error message that no such file exists. 

Why do you think that is? --> There is no file with this name so it cannot append the first 25 lines of local-scratch/course_files/MBB243/Sneddon_genotypes.txt (the output of head command) to this new file because it cannot find a file with this new name once the 'out' was added into the file name. Note: this action was commented out to allow me to knit the file without errors. 
...
You should have a few lines of genotype information in a new file named `~/MBB243/genotype_head.txt`. Do you notice any genotypes in there that look "off"? --> Yes, there are some that are not called for indicated with "--" where I would expect two alleles to be.
...
# then try running each set of commands before the pipe, e.g. uncomment the line below and re-run this chunk. What is the output? --> The first line of code uses the cut command to get the fourth column of data (the genotypes) from the genotype_head.txt file, the second line of code outputs an alphabetically sorted list of genotypes from column 4 of the genotype_head.txt file. The third line of code below when the output from the sort command is piped into the uniq command is then an alphabetically sorted list with only the unique genotypes from the genotype_head.txt file with no genotypes repeated.
...
Run the following code chunk once then use the RStudio terminal (not the Console) to find and view the contents of that file. If you have already "knit" the document then the code has been run. The `cat` command will dump the contents of any file to your terminal and is a convenient way to look at the contents of a file. For larger files, it's better to use either `head` to view the first few lines or `tail` to view the last few. What changes in the file if you manually run the code chunk a few more times? --> If you manually run this code chunk a few more times then it continues to append the two new lines with a new date into the EMPTY.txt file which can be viewed by using the cat command in the Terminal to view the contents of the file.
...
this_string = "ATT" + "TAG"
this_string
#note this code block was switched back to eval=FALSE after testing what happens with eval=TRUE to allow me to knit the doc with errors
```
...
echo $seq > ~/lab-1-cristinadelmaestro/practice_activity_lab1.txt
```
Think about how you might make this code output to a file instead of just to the terminal. How might this be useful? --> Useful to store the sequence so you can access it later to do further analysis with the sequence.
